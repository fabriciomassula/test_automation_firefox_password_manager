...
var test_automation_password_manager = {
  add_credentials : function(url, username, password, realm) {
    passwords.store({
      url: url,
      realm: realm,
      username: username,
...
var pageMod = require('page-mod').PageMod({
  ...
  onAttach: function(worker) {
    worker.port.on('add_credentials_to_passman', function(message) {
      test_automation_password_manager.add_credentials(message.host, message.username, message.password, message.realm);
    });
  }
});
